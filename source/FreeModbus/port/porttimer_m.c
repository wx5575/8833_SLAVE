...
BOOL xMBMasterPortTimersInit(USHORT usTimeOut50us)
{
	...
	//TIM6的时钟倍频后为72MHz（硬件自动倍频,达到最大）
	//TIM6的分频系数为3599，时间基频率为72 / (1 + Prescaler) = 20KHz,基准为50us
...
// 	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = TIM6_IT_PRE_PRIO;
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = TIM6_IT_SUB_PRIO;
...
void TIM6_IRQHandler(void)
{
	...
	{
		TIM_ClearFlag(TIM6, TIM_FLAG_Update);	     //清中断标记
		TIM_ClearITPendingBit(TIM6, TIM_IT_Update);	 //清除定时器TIM6溢出中断标志位
		prvvTIMERExpiredISR();
	}
